...
# CSS to add inside footer style
css_old = 'footer { background: #111; color: #aaa; padding: 56px 80px 24px; }'
css_new = '''.social-links { display: flex; gap: 10px; margin-top: 18px; }
.social-icon {
...
# Social icons HTML to inject after footer brand <p>
...
# Target: inject after footer brand <p>
inject_after = '      <p data-i18n="footer.desc">Solusi kemasan dan baking supply berkualitas tinggi untuk bisnis Anda di seluruh Indonesia.</p>'
...
for fname in files:
    try:
        ...
        # Add CSS
        if css_old in html and '.social-links' not in html:
            html = html.replace(css_old, css_new)
            changed = True

        # Add social HTML
        if inject_after in html and 'social-links' not in html:
        ...
        if changed:
            ...
            print(f'{fname} skipped (already updated or pattern not found)')
