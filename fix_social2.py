...
# New small social strip with inline styles on each element
new_strip = '''<!-- SOCIAL MEDIA STRIP -->
...
for fname in files:
    try:
        ...
        # Remove old social strip (with or without class)
        html = re.sub(
            r'<!-- SOCIAL MEDIA STRIP -->.*?</div>\s*\n',
            '', html, flags=re.DOTALL
        )

        # Insert new strip before <!-- FOOTER -->
        if '<!-- FOOTER -->' in html:
            html = html.replace('<!-- FOOTER -->', new_strip + '<!-- FOOTER -->')
        elif '<footer>' in html:
            html = html.replace('<footer>', new_strip + '<footer>', 1)
